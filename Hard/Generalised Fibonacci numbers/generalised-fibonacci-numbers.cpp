...
class Solution {
  public:
    ...
    long long genFibNum(long long a, long long b, long long c, long long n, long long m) {
        ...
        vector<vector<long long>> resultMatrix = {{1, 0, 0}, {0, 1, 0}, {0, 0, 1}};
        n -= 2;
        while (n > 0) {
            if (n & 1)
                resultMatrix = multiply(resultMatrix, mat, m);
            mat = multiply(mat, mat, m);
            n >>= 1;
        }
        return (resultMatrix[0][0] + resultMatrix[0][1] + c * resultMatrix[0][2]) % m;
    }
};
