...
class Solution {
  public:
    Node* bToDLL(Node* root) {
        // code here
        Node* head = nullptr;
        Node* prev = nullptr;  // to keep track of the last node
  ...
  private:
    void solve(Node* root, Node*& head, Node*& prev) {
        ...
        if (prev != nullptr) {
            prev->right = root; // set next pointer of the previous node
            root->left = prev; // set previous pointer of the current node
        } else {
            head = root; // leftmost node is head of dll
        }
        prev = root; // update prev to the curr node
